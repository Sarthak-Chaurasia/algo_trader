...
class mymap {
    private:
    int count=0,count0=0;int blabla=-2,countovr=0;
    ...
    void insert(vector< pair<string,int> > orders, int price, bool type,int multiplier,int id){
        ...
        if(maxprofit>0){
            ...
            if(countovr==multiplier){count0=0;}
        }
    }
};
class custom_map1 {
...
int main(int argc, char* argv[]) {
    if (argc != 2) {std::cout << "Oi KarGo what are you doing" << std::endl; return 0;}
    ...
    return 0;
        ...
        std::cout << "Bruh this is illegal" << std::endl;
        return 0;
    }
}
